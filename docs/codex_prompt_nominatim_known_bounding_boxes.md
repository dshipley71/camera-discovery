...
5. **Nominatim border geometry beats rectangular bboxes.** If Nominatim returns usable polygon or multipolygon geometry for the accepted geocoder candidate, use that geometry as the primary target boundary. For example, California should be represented by the California border polygon/multipolygon, not by the rectangular boundingbox, when Nominatim provides the border. The Nominatim `boundingbox` remains the fallback/search extent and must not replace a usable primary border geometry.
6. **Fallback hierarchy is mandatory.** Primary geometry is Nominatim polygon/multipolygon border. Fallback geometry is the rectangular Nominatim `boundingbox`. Last fallback geometry is a generic/default/synthetic padded bbox, used only when no usable Nominatim polygon/multipolygon or bbox exists. Generic fallback geometry must be clearly marked by provenance and must not be treated as authoritative Nominatim geometry. Deterministic padding around a known tiny Nominatim bbox is not the same as a no-geocoder generic fallback, but it must still be explicitly diagnosed.
...
Write a portable top-level `target_geometry.geojson` artifact so another GIS/map application can plot the resolved target boundary without parsing resolver logs. This GeoJSON must contain the primary Nominatim border geometry when available, the Nominatim bbox rectangle only when primary border geometry is unavailable, and last-fallback geometry only when no Nominatim geometry is usable. Include `target_geometry.geojson` in `review_artifacts.zip` and write status diagnostics such as `logs/target_geometry_geojson_status.json`.

### 8. Write portable target geometry artifacts

The artifacts package must contain a portable target-geometry GeoJSON file. Use the current source-aligned artifact name `target_geometry.geojson` unless the repository already has a stricter convention. This file must be a standard GeoJSON FeatureCollection suitable for plotting in another application.
